...
router.post('/:id', async (req, res)=>{
    ...
    if( encontrados)
    {
        res.send( {error:'Este pedido ya existe con el id: ' + required.id + ' Y es del usuario ' + encontrados.usuario});
        return;
    } 
    var pedido = req.body;
    pedido.id = filter.id;
    if(!pedido.usuario || !pedido.productos || !pedido.total || !pedido.estatus )
    {
